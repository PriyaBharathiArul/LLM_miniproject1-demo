...
def cosine_similarity(x, y):
    ...
    # Step 2: Compute the L2 norms (magnitudes) of x and y
    norm_x = la.norm(x)
    norm_y = la.norm(y)
    
    # Step 3: Handle edge case - if either vector is zero, return exp(0) = 1
    if norm_x == 0 or norm_y == 0:
        return np.exp(0)
    
    # Step 4: Compute cosine similarity = (x · y) / (||x|| * ||y||)
    cosine_sim = dot_product / (norm_x * norm_y)
    
    # Step 5: Exponentiate the cosine similarity
    # This converts range from [-1, 1] to [e^-1, e^1] = [0.368, 2.718]
    exponentiated_cosine_sim = np.exp(cosine_sim)
    
    return exponentiated_cosine_sim
...
def averaged_glove_embeddings_gdrive(sentence, word_index_dict, embeddings, model_type="50d"):
    ...
    # Step 2: Initialize counter for valid words (words that have embeddings)
    word_count = 0
    
    # Step 3: Loop through each word and accumulate embeddings
    for word in words:
        # Get the embedding for this word
        word_embedding = get_glove_embeddings(word, word_index_dict, embeddings, model_type)
        
        # Add this word's embedding to the total
        embedding += word_embedding
        
        # Check if the word had a valid embedding (not a zero vector)
        # np.any() returns True if at least one element is non-zero
        if np.any(word_embedding):
            word_count += 1
    
    # Step 4: Average the embeddings by dividing by the number of valid words
    # Protect against division by zero if no words had embeddings
    if word_count > 0:
...
def get_sorted_cosine_similarity(embeddings_metadata):
    ...
    categories = st.session_state.categories.split(" ")
    cosine_sim = {}
    text = st.session_state.get("text_search", "")
    ...
    if embeddings_metadata["embedding_model"] == "glove":
        ...
        # Step 2: Compare the sentence embedding with each category
        for i, category in enumerate(categories):
            # Get the embedding for this category word
            category_embedding = get_glove_embeddings(
                category, 
                word_index_dict, 
                embeddings, 
                model_type
            ...
            # Step 3: Compute cosine similarity between sentence and category
            similarity = cosine_similarity(sentence_embedding, category_embedding)
            
            # Store the similarity with the category index
            cosine_sim[i] = similarity  # Replace with your code
        ...
        # Step 2: Get the embedding for the input sentence using OpenAI
        sentence_embedding = get_openai_embeddings(text, model_name=model_name)
        
        # Step 3: Compare the sentence embedding with each category
        for i, category in enumerate(categories):
            # Get the cached category embedding directly
    ...
    cosine_sim_list = [(index, score) for index, score in cosine_sim.items()]
    
    # Step 2: Sort by similarity score in descending order (highest similarity first)
    # key=lambda x: x[1] means sort by the second element of each tuple (the score)
    # reverse=True means sort in descending order
    sorted_cosine_sim = sorted(cosine_sim_list, key=lambda x: x[1], reverse=True)
...
if __name__ == "__main__":
    ...
    if st.session_state.text_search:
        ...
        st.write("")
        st.write(
            "Demo developed by Priya Bharathi Arul (https://www.linkedin.com/in/priyabharathia1604/)"
        )
